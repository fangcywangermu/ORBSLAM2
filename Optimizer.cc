                                 ...
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
       ...
       const map<KeyFrame*,size_t> observations = pMP->GetObservations();
        ...
        //SET EDGES
        // 步骤3：向优化器添加投影边边 两端分别是地图点三维坐标和观察到该三维点坐标的关键帧
    ...
    * 只优化Frame的Tcw，不优化MapPoints的坐标(只优化当前帧的pose，地图点固定)
...
{   
    ...
    // 步骤4：开始优化，总共优化四次，每次优化后，将观测分为outlier和inlier，outlier不参与下次优化,计算出位姿后,对所有点重新分类内点/外点
...
{    
    ...
    // 步骤4：得到能被局部MapPoints观测到，但不属于局部关键帧的关键帧，这些关键帧在局部BA优化时不优化(注意图优化的关键帧节点分为两部分vNeighKFs和lFixedCameras都有)
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            // pKFi->mnBALocalForKF!=pKF->mnId表示局部关键帧，此条件是为了将局部关键帧排除在外
    ...
    // 步骤7：添加顶点：Pose of Fixed KeyFrame
    // lFixedCameras:全部设为固定  (设置观察到局部地图点的非局部关键帧集作为图优化的帧节点)
...
pLoopKF            闭环匹配上的关键帧
pCurKF             当前关键帧
NonCorrectedSim3   未经过Sim3传播调整过的关键帧位姿
CorrectedSim3      经过Sim3传播调整过的关键帧位姿
LoopConnections    因闭环时MapPoints调整而新生成的边
                                       ...
                                       const map<KeyFrame *, set<KeyFrame *> > &LoopConnections, const bool &bFixScale)
{
    ...
    // Set Loop edges
    // 设置回环边  边的两端分别是回环的两个端点,回环的初始端点是一个关键帧集合,末端点与该关键帧集合形成若干的边
    ...
    // 步骤6：设定优化后的位姿（位姿恢复  从相似变换矩阵变换到欧式变换矩阵）
    for(size_t i=0;i<vpKFs.size();i++)
    {
 ...
 * pKF1        KeyFrame
 * pKF2        KeyFrame
 * vpMatches1  两个关键帧的匹配关系
 * g2oS12      两个关键帧间的Sim3变换
 * th2         核函数阈值
 * bFixScale   是否优化尺度，单目进行尺度优化，双目不进行尺度优化
