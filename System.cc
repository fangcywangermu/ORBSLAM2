        ...
        mbDeactivateLocalizationMode(false)
{
    ...
    //相机内参、帧率、双目深度阈值等等，类似XXX.yaml这种文件类型
...
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp)    //运行系统为RGBD
{
    if(mSensor!=RGBD)
    {
...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp)    //运行系统为单目
{
    if(mSensor!=MONOCULAR)
    {
